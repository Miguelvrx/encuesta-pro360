...
class CrearEvaluacion extends Component
{
    ...
    public function mount($id = null)
    {
        ...
        $this->usuarios = User::where('estado', 'activo')->orderBy('name')->get();
    ...
    // PASO 4: Agregar calificador a un encuestado
    public function agregarCalificador($encuestadoIndex)
    {
        ...
        if (!$calificadorId || !$tipoRol) {
            session()->flash('error', 'Debes seleccionar un calificador y un tipo de relación.');
            return;
        }

        $calificador = User::find($calificadorId);
        if (!$calificador) {
            session()->flash('error', 'Calificador no encontrado.');
            return;
        }
        ...
        if ($existe) {
            session()->flash('error', 'Este calificador ya está asignado a este encuestado.');
            return;
        }
        ...
        $this->guardarPasoSilencioso();
        session()->flash('message', 'Calificador agregado exitosamente.');
    }

    // PASO 4: Eliminar calificador
    public function eliminarCalificador($encuestadoIndex, $calificadorIndex)
    {
        if (isset($this->calificadores[$encuestadoIndex][$calificadorIndex])) {
            ...
            $this->guardarPasoSilencioso();
            session()->flash('message', 'Calificador eliminado.');
        }
    }
    ...
    protected function validatePaso($paso)
    {
        ...
        switch ($paso) {
            ...
            case 3:
                $rules = [
                    "encuestados" => "required|array|min:1",
                ];
                break;
            case 4:
    ...
    public function enviarEvaluaciones()
    {
        try {
            ...
            // Crear registros en evaluacion_usuario
            foreach ($this->encuestados as $index => $encuestado) {
            ...
            // Aquí iría la lógica de envío de correos
            // $this->enviarCorreos();

            session()->flash('message', '¡Evaluaciones enviadas exitosamente!');
