    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="p-6 space-y-6">
                <div>
                    ...
                    @error('tipo_evaluacion') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @error('fecha_cierre') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                </div>
                ...
                <div>
                    ...
                    @error('descripcion_evaluacion') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>

        {{-- PASO 2: Configuración / Selección de Competencias --}}
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="p-6 space-y-6">
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <select wire:model.live="categoriaSeleccionada" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            ...
                            <option value="{{ $cat->id_categoria_competencia }}">{{ $cat->categoria }}</option>
                            @endforeach
                        </select>
                ...
                @error('competenciasSeleccionadas') <p class="text-sm text-red-600 mb-4">{{ $message }}</p> @enderror

                @if($competenciasPorCategoria->isEmpty())
                ...
                @else
                <div class="space-y-4">
                    @foreach($competenciasPorCategoria as $competencia)
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-3">
                                    <label class="flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                                            wire:click="toggleCompetencia({{ $competencia->id_competencia }})"
                                            @checked(in_array($competencia->id_competencia, $competenciasSeleccionadas))
                                        class="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                                    </label>
                                    <h4 class="text-base font-bold text-gray-900">{{ $competencia->nombre_competencia }}</h4>
                                </div>
                                <p class="mt-1 text-sm text-gray-600 ml-8">{{ $competencia->definicion_competencia }}</p>
                            </div>
                        </div>

                        @if($competencia->niveles->isNotEmpty())
                        <div class="ml-8 mt-3 space-y-2">
                            @foreach($competencia->niveles->sortByDesc(function($nivel) {
                            ...
                            <div class="flex items-center gap-3 text-xs">
                                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 font-semibold text-gray-700">
                                    {{ ['Excepcional' => 5, 'Supera las Expectativas' => 4, 'Competente' => 3, 'En Desarrollo' => 2, 'Requiere Apoyo' => 1][$nivel->nombre_nivel] ?? '?' }}
                                </span>
                                <span class="font-medium text-gray-700">{{ $nivel->nombre_nivel }}</span>
                                <span class="text-gray-500">{{ $nivel->descripcion_nivel }}</span>
                            </div>
                            @endforeach
                        </div>
                        @endif
                ...
                <div class="mt-6 p-4 bg-blue-50 border-l-4 border-blue-500 rounded-lg">
                    <div class="flex">
                        ...
                        <p class="ml-3 text-sm text-blue-700">
                            <strong>Competencias seleccionadas:</strong> {{ count($competenciasSeleccionadas) }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        {{-- PASO 5: Revisión y Asignaciones --}}
        @elseif($paso_actual == 5)
        <div class="space-y-6">
            {{-- Resumen --}}
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-3">
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <p class="text-sm"><strong>Nombre:</strong> {{ $tipo_evaluacion }}</p>
                            <p class="text-sm"><strong>Tipo:</strong> {{ $tipo_evaluacion }}</p>
                            <p class="text-sm"><strong>Periodo:</strong> {{ \Carbon\Carbon::parse($fecha_inicio)->format('d M Y') }} - {{ \Carbon\Carbon::parse($fecha_cierre)->format('d M Y') }}</p>
                        </div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <p class="text-sm"><strong>Competencia:</strong> {{ count($competenciasSeleccionadas) }} seleccionadas</p>
                            <p class="text-sm"><strong>Preguntas:</strong> {{ collect($competenciasSeleccionadas)->sum(function($id) {
                    ...
                    <div class="space-y-3">
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <p class="text-sm"><strong>Total:</strong> {{ count($encuestados) }} {{ count($encuestados) == 1 ? 'persona' : 'personas' }}</p>
                        </div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <p class="text-sm"><strong>Total:</strong> {{ collect($calificadores)->flatten(1)->count() }} Asignados</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6">
                    <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6">
                        <p class="text-sm text-blue-800">
                            <strong>Asunto:</strong> Invitación - Evaluación 360° {{ $tipo_evaluacion }}
                        </p>
                    </div>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                        <p class="text-sm text-gray-700 leading-relaxed">
                            ...
                            @if(count($encuestados) > 0)
                            <strong>{{ $encuestados[0]['nombre'] }}</strong>
                            @endif
                        </p>
                        <p class="text-sm text-gray-700 mt-4">
                            <strong>Período:</strong> {{ \Carbon\Carbon::parse($fecha_inicio)->format('d M Y') }} - {{ \Carbon\Carbon::parse($fecha_cierre)->format('d M Y') }}
                        </p>
                        <div class="mt-6">
        ...
        <div class="flex justify-between mt-8">
            @if($paso_actual > 1 && $paso_actual < 5)
