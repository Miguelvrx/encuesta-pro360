    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-xl p-6 shadow-lg">
                ...
                <div class="text-3xl font-bold">{{ $usuarios->total() }}</div>
            </div>
            @foreach($roles->take(3) as $rol)
            <div class="bg-white rounded-xl p-6 shadow-lg border-l-4 border-indigo-500">
                <div class="text-sm text-gray-600 mb-1">{{ $rol->name }}</div>
                <div class="text-3xl font-bold text-indigo-600">{{ $rol->users->count() }}</div>
            </div>
            @endforeach
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm">
                                    <div class="font-medium text-gray-900">{{ $usuario->puesto ?? 'Sin puesto' }}</div>
                                    <div class="text-gray-500">
                                        {{ $usuario->departamento->nombre_departamento ?? 'Sin departamento' }}
                                    </div>
                                    <div class="text-xs text-gray-400">
                                        {{ $usuario->departamento->empresa->nombre_comercial ?? '' }}
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="abrirModal({{ $usuario->id }})"
                                    ...
                                    <button wire:click="removerTodosLosRoles({{ $usuario->id }})"
                                        wire:confirm="¿Remover todos los roles de {{ $usuario->name }}?"
        ...
        <!-- Modal Asignar Roles -->
        @if($modalAbierto && $usuarioSeleccionado)
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="flex items-start justify-between mb-6">
                            <div class="flex items-center space-x-4">
                                @if($usuarioSeleccionado->profile_photo_path)
                                <img class="h-16 w-16 rounded-full object-cover"
                                    src="{{ asset('storage/' . $usuarioSeleccionado->profile_photo_path) }}"
                                    alt="{{ $usuarioSeleccionado->name }}">
                                @else
                                <div class="h-16 w-16 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-2xl">
                                    {{ substr($usuarioSeleccionado->name, 0, 1) }}
                                </div>
                                @endif
                                <div>
                                    <h3 class="text-2xl font-bold text-gray-900">
                                        {{ $usuarioSeleccionado->name }} {{ $usuarioSeleccionado->primer_apellido }}
                                    </h3>
                                    <p class="text-sm text-gray-500">{{ $usuarioSeleccionado->email }}</p>
                                    <p class="text-xs text-gray-400">{{ $usuarioSeleccionado->puesto }}</p>
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="grid grid-cols-1 gap-3 max-h-96 overflow-y-auto p-4 bg-gray-50 rounded-lg">
                                    ...
                                    <label class="flex items-start space-x-3 cursor-pointer hover:bg-white p-4 rounded-lg border-2 transition-all
                                        {{ in_array($rol->name, $rolesSeleccionados) ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200' }}">
                                        <input type="checkbox"
                                            wire:model="rolesSeleccionados"
                                            value="{{ $rol->name }}"
                                            class="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 mt-1">
                                        <div class="flex-1">
                                            <div class="font-semibold text-gray-900">{{ $rol->name }}</div>
                                            <div class="text-xs text-gray-500 mt-1">
                                                {{ $rol->permissions->count() }} permisos asignados
                                            </div>
                                            <div class="flex flex-wrap gap-1 mt-2">
                                                @foreach($rol->permissions->take(4) as $permiso)
                                ...
                                <p class="mt-3 text-sm text-gray-600">
                                    Roles seleccionados: <strong class="text-indigo-600">{{ count($rolesSeleccionados) }}</strong>
                                </p>
                            </div>
