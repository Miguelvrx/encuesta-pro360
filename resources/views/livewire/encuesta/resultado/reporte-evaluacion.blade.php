    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
            ...
            <div>
                ...
                <div class="flex space-x-4">
                    <button wire:click="$set('tipoVisualizacion', 'tabla')" class="px-4 py-2 rounded-md text-sm font-medium {{ $tipoVisualizacion === 'tabla' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                        Tabla
                    </button>
                    <button wire:click="$set('tipoVisualizacion', 'grafico')" class="px-4 py-2 rounded-md text-sm font-medium {{ $tipoVisualizacion === 'grafico' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                        Gráfico
                    </button>
                </div>
            </div>
        ...
        @else
            ...
            @elseif($tipoVisualizacion === 'grafico')
                <!-- Vista de Gráfico -->
                <div class="bg-white rounded-xl shadow-lg p-6">
                    @if($usuarioEvaluadoSeleccionado && isset($graficos['radar']))
                        <h3 class="text-lg font-bold text-gray-800 mb-4">Promedio por Competencia para {{ $resultados[$usuarioEvaluadoSeleccionado]['nombre'] }}</h3>
                        <div style="width: 100%; max-width: 700px; margin: auto;">
                            <canvas id="radarChart"></canvas>
                        </div>
                        @if(isset($graficos['barras_roles']))
                            <h3 class="text-lg font-bold text-gray-800 mt-8 mb-4">Promedio por Rol y Competencia para {{ $resultados[$usuarioEvaluadoSeleccionado]['nombre'] }}</h3>
                            <div style="width: 100%; max-width: 900px; margin: auto;">
                                <canvas id="barrasRolesChart"></canvas>
                            </div>
                        @endif
                    @elseif($tipoReporte === 'general' && isset($graficos['barras_generales']))
                        <h3 class="text-lg font-bold text-gray-800 mb-4">Promedio General por Evaluado</h3>
                        <div style="width: 100%; max-width: 900px; margin: auto;">
                            <canvas id="barrasGeneralesChart"></canvas>
                        </div>
                    @elseif($tipoReporte === 'por_competencia' && isset($graficos['barras_competencias']))
